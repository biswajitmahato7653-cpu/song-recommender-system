...
songs_show = df.head(20)

for _, row in songs_show.iterrows():
    ...
    youtube_query = f"{song_name} {artist}".replace(" ", "+")
    ...
    with col2:
        st.markdown(
            f"""
            <h2 style='margin-bottom:0'>{song_name}</h2>
            <p style='color:#00eaff;font-size:22px'>{artist}</p>
            """,
            unsafe_allow_html=True
        )
